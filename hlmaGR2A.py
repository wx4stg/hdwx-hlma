...
def makeOneMinPlots(lmaFilePath):
    ...
    # 
    lmaData["pyDatetime"] = pd.to_datetime(lmaData["Datetime"])
    # Get time of the plot for metadata
    startTimeOfPlot = lmaData["pyDatetime"][0].replace(microsecond=0)
    ...
    gr2aSavePath = path.join(basePath, "output", gr2aProductPath, "1min-src.txt")
    # Create parent dir
    ...
    phpText = "<?php\n    header('Content-type: text/plain');\n    echo file_get_contents(\"./1min-src.txt\")\n?>"
    with open(path.join(basePath, "output", gr2aProductPath, "1min-src.php"), "w") as phpWrite:
...
if __name__ == "__main__":
    ...
    if path.exists(oneMinMetadataPath):
        ...
        if lastOneMinMetadata["productFrames"][0]["valid"] < int(lastOneMinFileDt.strftime("%Y%m%d%H%M")):
            makeOneMinPlots(path.join(inputPath, lastOneMinFile))
    else:
